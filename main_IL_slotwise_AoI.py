...
shapeud = (num_tests, number_of_users)
...
REWmean_mm = np.memmap(os.path.join(Out_dir, 'Rew_u_mean.dat'),
                       dtype=np.float32, mode='w+', shape=shape1d)
AOI_test_mm = np.memmap(os.path.join(Out_dir, 'AOI_test.dat'), dtype=np.float32, mode='w+', shape=shapeud)
...
for t in range(1, test + 1):
    ...
    for it_ind in range(0, iterations):
        ...
        EH_Raw = np.empty(number_of_users,
                          dtype=float)  # to save raw battery capacity of all users at current iteration/frame
        # EH_Raw[:, 0] = EH_Raw[:, 0] + 0.05
        BT_Dis = np.empty(number_of_users,
                          dtype=int)  # to save discrete battery capacity of all users at current iteration/frame
        CH_Raw = np.empty(number_of_users,
                          dtype=complex)  # to save raw channel gamma of all users at current iteration/frame
        CH_Dis = np.empty(number_of_users,
                          dtype=int)  # to save discrete channel gamma of all users at current iteration/frame
        ...
        for d in range(np.size(users)):
            ...
            if randx <= epsilon:  # Random Action Selection
                ...
                if bt_units == 0:
                    ...
                    range_slot = np.array(range(0, number_of_slots),
                                          dtype=int)  # to ask it to make a choice between first and last slot
                    ...
                    user_action = np.random.choice(range_action, size=1,
                                                   p=prob_dist)  # choose an action based on prob_dist
                    user_action = user_action[0]
                    if user_action > a.size:
                        user_action = a.size
                    if user_action > BT_Dis[d]:
                        user_action = BT_Dis[d]
                    slot_indices = np.random.choice(range_slot, size=user_action,
                                                    replace=False)  # choose random slots to send the packet
                ...
                if action == 0:
                    ...
                    if action > BT_Dis[d]:
                        action = BT_Dis[d]
                    ind_u = np.random.choice(number_of_slots, size=action,
                                             replace=False)  # rd.sample(range(number_of_slots), action)
        ...
        # apply SIC
        decoded_users = capture_effect_SIC_realtime(slot_aloc_f, number_of_slots, number_of_users, G_Raw,
                                                    verbose=False)  # Rearly-n method
        ...
        # Track AoI for each slot in the frame

        for s in range(number_of_slots):
            for u in range(number_of_users):
                tx_slot = decoded_users[u, 1]
                rx_slot = decoded_users[u, 0]

                if rx_slot == s:  # Decoded exactly in this slot
                    if tx_slot == -1:
                        users[u].AOI += 1  # Not decoded, keep aging
                    else:
                        users[u].AOI = rx_slot - tx_slot + 1  # Reset AoI
                else:
                    users[u].AOI += 1  # Aging continues
                AOI_cumsum [u] += users[u].AOI
                #AOI_cumsum [u] = AOI_cumsum [u] / ((s+1) * (it_ind + 1) )

        Af_aoi = AOI_cumsum #AoI at the end of the frame
        AOI_test_iter[ it_ind, :] = Af_aoi
        for u in range(number_of_users):
            Rew_U[u] = get_rew(Bf_aoi[u], users[u].AOI, BT_Dis[u], AC_users[u], CH_Dis[u])
        ...
        if (it_ind + 1) % Batch_size == 0:
            ...
            dist = np.random.uniform(dist_min, dist_max, size=number_of_users)
            BT_Dis_next = np.empty(number_of_users,
                                   dtype=int)  # to save discrete battery capacity of all users at current iteration/frame
            CH_Raw_next = np.empty(number_of_users,
                                   dtype=complex)  # to save raw channel gamma of all users at current iteration/frame
            CH_Dis_next = np.empty(number_of_users,
                                   dtype=int)  # to save discrete channel gamma of all users at current iteration/frame
            ...
            for u in range(number_of_users):
                ...
                temporal_difference = Rew_U[u] + discount_factor * best_next_action_value - q_tables[
                    u, row_act, AC_users[u]]
...
for mm in (AOI_mm, AOI_test_mm, AC_mm, CH_mm, BT_mm, REW_mm, G_mm, CHRAW_mm,
