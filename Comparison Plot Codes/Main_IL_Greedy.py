...
L       = 5
Out_dir  = f"GRDPDNOMA_S_{number_of_slots}_U_{number_of_users}_PL{L}_BT{max_bt}_T{test}"
# S = ((), dtype=float)
...
_RNG = np.random.default_rng()  # reuse your RNG, or seed as you like

def greedy_feasible_by_energy(mask: np.ndarray,
                              e_k_now: np.ndarray,
                              number_of_slots: int,
                              require_tx: bool = True) -> tuple[int, int]:
    """
    Pick (l, k) in the feasible set that maximizes 'energy used' = min(l, S) * e_k.
    - mask: (l_cap+1, L) boolean feasibility over (replicas, level)
    - e_k_now: (L,) per-replica energy for each level at this user's current γ
    - number_of_slots: cap on replicas that can be placed this frame
    - require_tx=True → exclude l=0 (always transmit when feasible)
    Returns (l_best, k_best). If none feasible (or only l=0 with require_tx), returns (0, 0).
    """
    feas = np.argwhere(mask)  # rows: [l, k]
    if feas.size == 0:
        return 0, 0
    ...
    for l, k in feas:
        l = int(l); k = int(k)
        if require_tx and l == 0:
            continue
        # enforce slot cap in the objective so we don't pick l > S
        l_eff = min(l, int(number_of_slots))
        if l_eff <= 0:
            continue
        score = float(l_eff) * float(e_k_now[k])  # total energy = replicas * per-replica energy
        if score > best_score:
            best_score = score
            best_cands = [(l_eff, k)]
        elif score == best_score:
            best_cands.append((l_eff, k))

    if not best_cands:
        return 0, 0

    # tie-break randomly
    return best_cands[_RNG.integers(len(best_cands))]
...
for t in range(1, test+1):
    ...
    for it_ind in range(0, iterations):
        ...
        # ===================== GREEDY (max energy) PER-USER DECISION BLOCK ======================
        for d in range(np.size(users)):
            ...
            # --- GREEDY over feasible: maximize total energy this frame ----------------
            if not mask_now.any():
                l_sel, k_idx = 0, 0
            else:
                # require_tx=True → never choose l=0 if any TX is feasible
                l_sel, k_idx = greedy_feasible_by_energy(mask_now, e_k_now, number_of_slots, require_tx=True)
            ...
            # Energy needed for this (l_sel, k_idx)
            # (l_sel already respects slot cap in the selector)
            e_needed = float(l_sel) * float(e_k_now[k_idx])

            # Final guard (should pass if mask was computed from continuous battery)
            if users[d].get_battery_level() + 1e-12 < e_needed:
                # Not enough battery → skip TX (do NOT mark slots)
                AC_users[d] = 0
                total_energy[d] = 0.0
                continue

            # Choose l_sel distinct slots to place replicas
            slot_indices = _RNG.choice(np.arange(number_of_slots, dtype=int),
                                       size=int(l_sel), replace=False)

            # Mark the chosen slots with power level id: 1..L (1 = highest)
            power_level_to_write = k_idx + 1
            slot_aloc_f[d, slot_indices] = power_level_to_write

            # Debit continuous energy (or legacy fallback)
            total_energy[d] = e_needed
            debit_energy(users[d], e_needed, upsilon_legacy=upsilon)

            # Book-keeping
            AC_users[d] = int(l_sel)
        # =================== END GREEDY (max energy) PER-USER DECISION BLOCK =====================
