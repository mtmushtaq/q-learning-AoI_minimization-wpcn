...
methods = {
    'IL':  {'base_dir': Path(r"C:\Users\Tauseef\OneDrive - Politecnico di Bari\AOI Q Learning Infocom\Data July\IL_U100"),  'linestyle':'--', 'marker':'s'},
    'JAL': {'base_dir': Path(r"C:\Users\Tauseef\OneDrive - Politecnico di Bari\AOI Q Learning Infocom\Data July\JAL_U100"), 'linestyle':'-',  'marker':'o'}
}
...
def plot_policy_evolution(save_dir, filename):
    ...
    fig, axes = plt.subplots(2,2,figsize=(10,8),dpi=600,sharey=True)
        ...
        for name,cfg in methods.items():
            ...
            for i,a in enumerate(all_actions):
                ax.plot(x, probs[:,i],
                        ...
                        label=fr"$\mu({a}; t, G)$ " if name=='IL' else None,
                        alpha=0.8)
        # subplot annotations
        ax.set_title(fr"$G$={g}",fontsize=15,fontweight='light')
        if idx%2==0:
            ax.set_ylabel(fr"$\boldsymbol{{\mu}}(t;G)$", fontsize=15)
        if idx//2==1:
...
if __name__=='__main__':
    plot_policy_evolution('PolicyPlots_U100_4', 'policy_evolution_IL_JAL_U100_43.pdf')
