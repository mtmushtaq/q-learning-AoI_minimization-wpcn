...
def plot_battery_all_gains_log(save_dir, output_name, smoothing_window=3):
    ...
    print("\n📊 Final Battery Values for Each Gain (Test 100):\n")
    ...
    for idx, (g, slot) in enumerate(zip(gains, slots)):
        ...
        for method, cfg in table.items():
            ...
            try:
                arr = load_test_matrix_npy('BT_user_tests', str(folder))[:tests_to_plot, :, :]  # shape (tests, users, time)
                if method == 'Dist':
                    arr = arr / 2.0

                smooth = pd.Series(arr.mean(axis=(1, 2))).rolling(
                    window=smoothing_window, min_periods=1, center=True
                ).mean()

                final_value = smooth.iloc[-1]
                print(f"  {legend_name.get(method, method)}: {final_value:.3f}")
                ...
                print(f"  ⚠️ {legend_name.get(method, method)} → Failed to load: {e}")
                continue
        ...
        if col == 0:
            ax.set_ylabel(r"$\bar{\zeta}(t; G)$", fontsize=12, fontweight='bold')
        if row == 1:
            ax.set_xlabel('Test', fontsize=12, fontweight='bold')
...
if __name__ == '__main__':
    plot_battery_all_gains_log('BatteryPlots_all', 'IRSA_Log8')
