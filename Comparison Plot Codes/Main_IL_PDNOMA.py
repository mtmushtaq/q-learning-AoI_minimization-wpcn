...
L       = 2
Out_dir  = f"TILPDNOMA_S_{number_of_slots}_U_{number_of_users}_PL{L}_BT{max_bt}_T{test}"
# S = ((), dtype=float)
...
def build_mu_ladder(Gamma: float, N0: float, L: int) -> np.ndarray:
    """
    μ_k = Γ N0 (1+Γ)^(L-k), k = 1..L
    Returns array indexed from 0..L-1 mapping to k=1..L.
    """
    mu = np.empty(L, dtype=float)
    for k in range(1, L+1):
        mu[k-1] = Gamma * ((1.0 + Gamma) ** (L - k))
    return mu

mu_levels = build_mu_ladder(Gamma, N0, L)   # shape (L,)

# --- Energy per replica & masks ----------------------------------------------
def per_level_energy(mu_levels: np.ndarray, gamma_rep: float, delta_slot: float) -> np.ndarray:
    """
    e_k = (μ_k / γ_rep) * δ  for each level k (index 0..L-1 -> k=1..L).
    """
    g = max(gamma_rep, 1e-12)
    return (mu_levels / g) * delta_slot

def lmax_per_level_cont(omega_continuous, e_k: np.ndarray, l_cap: int) -> np.ndarray:
    """
    Compute per-level max replicas using the *continuous* battery (same units as e_k).
...
def random_feasible_action(mask: np.ndarray) -> tuple[int, int]:
    """
    Uniform random (l,k) among feasible actions only.
    """
    feas_idx = np.argwhere(mask)
    if feas_idx.size == 0:
        return 0, 0
    i = np.random.randint(len(feas_idx))
    return int(feas_idx[i, 0]), int(feas_idx[i, 1])
...
def debit_energy(user_obj, energy: float):
    """
    Prefer a real energy-based debit if you have it; otherwise convert to 'units'
    and call decrease_EH(int_units). Clip at available battery.
    """
    if hasattr(user_obj, "spend_energy"):           # optional modern API
        user_obj.spend_energy(float(energy))
    else:
        units = energy_to_legacy_units(float(energy), upsilon)
        units_int = int(np.ceil(units))             # conservative: round up
        units_int = max(units_int, 0)
        user_obj.decrease_EH(units_int)
...
for t in range(1, test+1):
    ...
    for it_ind in range(0, iterations):
        ...
        for d in range(np.size(users)):
            ...
            BT_Dis[d] = users[d].BT_units(battery_bins)  # current battery (energy units)
            CH_Dis[d] = get_channel(G_Raw[d])  # CSI bin index (0..C-1)
            row_act = get_row(BT_Dis[d], CH_Dis[d])  # state index in Q

            # --- Build feasibility at current state (battery+CSI aware) ---------------
            gamma_rep = to_scalar(G_Raw[d])  # use actual γ this frame
            e_k_now = per_level_energy(mu_levels, gamma_rep, delta_slot)  # shape (L,)
            omega_now = users[d].get_battery_level()
            lmax_now = lmax_per_level_cont(omega_now, e_k_now, l_cap)   # shape (L,)
            mask_now = build_mask(lmax_now, l_cap)  # (l_cap+1, L)

            # --- Choose (replicas, power) via explore/exploit on the FEASIBLE set -----
            if not mask_now.any():
                # no feasible action -> no transmission
                l_sel, k_idx = 0, 0
            else:
                r = float(np.round(np.random.random(), 5))
                if r <= epsilon:
                    explore += 1
                    l_sel, k_idx = random_feasible_action(mask_now)  # UNIFORM over feasible
                else:
                    exploit += 1
                    Q_now_slice = q_tables[d, row_act, :, :]  # (l_cap+1, L)
                    l_sel, k_idx = masked_argmax(Q_now_slice, mask_now)  # GREEDY over feasible

            # --- Allocate exactly l_sel slots and debit energy ------------------------
            if l_sel <= 0:
                # Optional: clear the row if you rebuild from zeros each frame
                # slot_aloc_f[d, :] = 0
                AC_users[d] = 0
            else:
                # Pick l_sel distinct slot columns for this user
                ...
                # Write ONLY the chosen slots with the power level id: 1..L (1 = highest)
                power_level_to_write = k_idx + 1
                # If you rebuild slot_aloc_f from zeros each frame, no need to clear first.
                # Otherwise, ensure you don't overwrite other slots unintentionally:
                # slot_aloc_f[d, :] = 0   # uncomment if you want to reset row every frame
                total_energy[d] = float(l_sel) * float(e_k_now[k_idx])
                if users[d].get_battery_level() < total_energy[d]:
                    slot_aloc_f[d, slot_indices] =0
                slot_aloc_f[d, slot_indices] = power_level_to_write

                # Battery debit = replicas * per-replica energy at chosen level

                # Prefer energy-based debit; fallback to legacy decrease_EH if needed
                if hasattr(users[d], "spend_energy"):
                    users[d].spend_energy(total_energy[d])
                else:
                    users[d].decrease_EH_cont(int(total_energy[d]) )

                # Book-keeping
                AC_users[d] = int(l_sel)

            # NOTE: your TD target / reward update should later use the SAME masking logic
            # at the NEXT state (build next-mask and take masked max for the Bellman backup).
